...
import { getNewProducts } from "@/lib/api/product.api";
...
export default function Page() {
    ...
    useEffect(() => {
        async function loadProducts() {
            try {
                setLoading(true);
                const { data, error } = await getNewProducts();
                
                if (error) throw error;
                
                setBaseProducts(adaptProductsToUI(data));
            } catch (err) {
                ...
                <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between mb-6 md:mb-8">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="h-1 w-10 bg-sky-500 rounded-full" />
                        </div>
